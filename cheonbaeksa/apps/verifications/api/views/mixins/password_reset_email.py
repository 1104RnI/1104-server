...
# Main Section
class EmailVerificationPasswordResetViewMixin:
    @swagger_auto_schema(**swagger_decorator(tag='이메일',
                                             id='비밀번호 변경 인증 이메일 전송',
                                             description="""

                                             비밀번호 변경을 위한 이메일 인증 코드를 생성하여 사용자에게 전송합니다.
    ...
    @action(detail=False, methods=['post'], url_path='password-reset-verification', url_name='password_reset_email')
    def password_reset_email(self, request, *args, **kwargs):
        ...
        # 이메일 전송 로직
        subject = '1104 R&I 비밀번호 변경 이메일 인증 코드'
